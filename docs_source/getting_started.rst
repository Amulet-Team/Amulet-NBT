...
.. code-block:: python
   ...
   named_tag = amulet_nbt.load_one(b'<nbt file bytes>')  # from a bytes object

   # Note that Java Edition usually uses compressed modified UTF-8.
   java_named_tag = amulet_nbt.load_one(
   ...
   bedrock_named_tag = amulet_nbt.load_one(

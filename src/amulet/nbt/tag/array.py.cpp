...
#define PyArray(CLSNAME, ELEMENTCLS, BITCOUNT, TAGID)                                                                      \
    ...
    CLSNAME.def(                                                                                                           \
        py::init([](py::typing::Iterable<ELEMENTCLS> value) {                                                              \
            std::vector<ELEMENTCLS> vec;                                                                                   \
            for (py::handle handle : value) {                                                                              \
                py::object obj;                                                                                            \
                if (!PyIndex_Check(handle.ptr())) {                                                                        \
                    obj = py::reinterpret_steal<py::object>(PyNumber_Long(handle.ptr()));                                  \
                    if (!obj) {                                                                                            \
                        throw py::error_already_set();                                                                     \
                    }                                                                                                      \
                    handle = obj;                                                                                          \
                }                                                                                                          \
                unsigned long long i = PyLong_AsUnsignedLongLongMask(handle.ptr());                                        \
                vec.push_back(static_cast<ELEMENTCLS>(i));                                                                 \
            }                                                                                                              \
            return std::make_shared<Amulet::NBT::CLSNAME>(std::move(vec));                                                 \
        }),                                                                                                                \

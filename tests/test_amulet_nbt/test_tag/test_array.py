...
class ArrayTagTestCase(AbstractBaseMutableTagTestCase, unittest.TestCase):
    def test_constructor(self) -> None:
        ...
        self.assertEqual(ByteArrayTag([-2**7, 2**7-1]), ByteArrayTag([2**7, -2**7-1]))
        self.assertEqual(IntArrayTag([-2**31, 2**31-1]), IntArrayTag([2**31, -2**31-1]))
        self.assertEqual(LongArrayTag([-2**63, 2**63-1]), LongArrayTag([2**63, -2**63-1]))
